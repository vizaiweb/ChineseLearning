...
const SentenceBuilder = {
    ...
    init() {
        this.bindEvents();
        console.log('✅ SentenceBuilder initialized');
    },
    ...
    loadFromArticle(article) {
        ...
        this.totalCount = sentences.length;
        this.currentIndex = 0;
        this.stats = { total: 0, correct: 0 };
        this.isOpen = false;
        document.getElementById('builderContent').classList.remove('open');
        this.loadSentence(0);
        this.updateStats();
    },
    ...
    loadSentence(index) {
        ...
        if (index < 0) index = 0;
        if (index >= this.sentences.length) index = this.sentences.length - 1;

        this.currentIndex = index;
        const sentence = this.sentences[index];
        this.correctSentence = sentence;
        const chars = sentence.split('');
        this.sourceChars = this.shuffleArray(chars);
        this.targetChars = chars.map(() => null);
        this.isAnswered = false;

        const hintEl = document.getElementById('hintText');
        if (hintEl) hintEl.textContent = `📖 ${sentence}`;
        ...
        if (feedback) {
            feedback.textContent = '💡 將下方單字拖曳到上方答案區';
    ...
    render() {
        ...
        let sourceHtml = '';
        if (this.sourceChars.length === 0) {
            sourceHtml = '<span style="color:#94a3b8; font-size:14px;">(所有文字已放置)</span>';
        } else {
            sourceHtml = this.sourceChars.map((char, idx) => {
                return `<span class="char-token source-token" data-source-index="${idx}">${this.escapeHtml(char)}</span>`;
            }).join('');
        }
        sourceDropzone.innerHTML = sourceHtml;

        let targetHtml = this.targetChars.map((char, idx) => {
            if (char !== null) {
                return `<span class="char-token target-token" data-target-index="${idx}">${this.escapeHtml(char)}</span>`;
            } else {
                return `<span class="char-token target-token empty-slot" data-target-index="${idx}">?</span>`;
            }
        }).join('');
        targetDropzone.innerHTML = targetHtml;

        const nextBtn = document.getElementById('nextBtn');
        if (nextBtn) {
            nextBtn.disabled = (this.currentIndex >= this.totalCount - 1);
        }

        this.bindDragEvents();

        if (this.isAnswered) {
            const isCorrect = this.checkAnswerInternal();
            const feedback = document.getElementById('feedbackMsg');
            if (feedback) {
                if (isCorrect) {
                    feedback.textContent = '✅ 完全正確！';
    ...
    // 內部檢查答案
    checkAnswerInternal() {
        const correct = this.correctSentence.split('');
        const current = this.targetChars;
        if (current.some(c => c === null)) return false;
        for (let i = 0; i < correct.length; i++) {
            if (current[i] !== correct[i]) return false;
        }
        return true;
    },
    ...
    updateStats() {
        ...
        if (statsEl) {
            statsEl.innerHTML =
                `📌 第 <span class="stat-number">${current}</span> / <span class="stat-number">${this.totalCount}</span> 句 &nbsp;| ✅ 答對率: <span class="stat-number">${total > 0 ? rate + '%' : '--%'}</span>`;
        }
    },
    ...
    checkAnswer() {
        ...
        if (isCorrect) {
            ...
            setTimeout(() => { this.playBeep(1100, 0.15); }, 150);
        } else {
            const feedback = document.getElementById('feedbackMsg');
            if (feedback) {
                feedback.textContent = '❌ 再試一次！';
    ...
    shuffleChars() {
        ...
        const allChars = this.correctSentence.split('');
        const remaining = [];
        const targetCopy = [...this.targetChars];
        ...
        if (remaining.length > 0) {
            this.sourceChars = this.shuffleArray(remaining);
        } else {
            this.sourceChars = [];
        }
        const newTarget = allChars.map((ch, idx) => {
            return this.targetChars[idx] !== null ? this.targetChars[idx] : null;
        });
        this.targetChars = newTarget;
        this.isAnswered = false;
        const feedback = document.getElementById('feedbackMsg');
        if (feedback) {
            feedback.textContent = '💡 將下方單字拖曳到上方答案區';
    ...
    // 朗讀提示句子
    speakHint() {
        const sentence = this.correctSentence;
        if (!sentence) return;
        // 直接呼叫 speakCantoneseOnce（已在全域定義）
        if (typeof speakCantoneseOnce === 'function') {
            speakCantoneseOnce(sentence, () => {});
        } else if (typeof playCantoneseOnly === 'function') {
            playCantoneseOnly(sentence);
        }
    },
    ...
    toggle() {
        ...
        if (this.isOpen) {
            ...
            // 檢查文章是否隱藏
            if (typeof isArticleHidden !== 'undefined' && isArticleHidden) {
                content.classList.remove('open');
                alert('文章已隱藏，請先顯示文章');
                this.isOpen = false;
                return;
            }
            if (this.sentences.length === 0 && typeof articles !== 'undefined' && articles.length > 0) {
                const art = articles[currentArticleIndex || 0];
                if (art && art.content) {
                    this.loadFromArticle(art);
                }
            }
        } else {
            content.classList.remove('open');
    ...
    bindEvents() {
        ...
        if (shuffleBtn) shuffleBtn.onclick = () => this.shuffleChars();
        if (checkBtn) checkBtn.onclick = () => this.checkAnswer();
        if (nextBtn) nextBtn.onclick = () => this.nextSentence();
        if (listenBtn) listenBtn.onclick = () => this.speakHint();

        // 監聽文章切換（如果 showArticle 已定義）
        if (typeof window.showArticle === 'function') {
            const origShowArticle = window.showArticle;
            window.showArticle = function(index) {
                origShowArticle(index);
                const art = window.articles[index];
                if (art && art.content) {
                    SentenceBuilder.loadFromArticle(art);
                }
            };
        }
    },
    ...
    bindDragEvents() {
        ...
        function getDragData(e) {
            ...
            if (type === 'target' && self.targetChars[index] === null) return null;
        ...
        function onDragMove(e) {
            ...
            const dx = clientX - self.dragData.startX;
            const dy = clientY - self.dragData.startY;
            self.dragData.element.style.transform = 'translate(' + dx + 'px, ' + dy + 'px)';

            document.querySelectorAll('.char-token, .empty-slot').forEach(t => t.classList.remove('drag-over-me'));
            ...
            allTargets.forEach(el => {
                if (el === self.dragData.element) return;
            ...
            self.dragData.target = { type: targetType, index: targetIndex, element: targetElement };
        }

        function onDragEnd(e) {
            if (!self.dragData) return;
            const el = self.dragData.element;
            ...
            const sourceType = self.dragData.type;
            const sourceIndex = self.dragData.index;
            const target = self.dragData.target;
            ...
            if (target && target.type !== null && target.index !== null) {
                ...
                const sourceChars = self.sourceChars;
                const targetChars = self.targetChars;
                let sourceWord, targetWord;

                if (sourceType === 'source') {
                    sourceWord = sourceChars[sourceIndex];
                    if (sourceWord === undefined) { self.dragData = null;
                        self.render(); return; }
                } else {
                    sourceWord = targetChars[sourceIndex];
                    if (sourceWord === null) { self.dragData = null;
                        self.render(); return; }
                }

                if (targetType === 'source') {
                    targetWord = sourceChars[targetIndex];
                    if (targetWord === undefined) { self.dragData = null;
                        self.render(); return; }
                } else {
                    targetWord = targetChars[targetIndex];
                }

                if (sourceType === 'source' && targetType === 'source') {
                    const temp = sourceChars[sourceIndex];
                    sourceChars[sourceIndex] = sourceChars[targetIndex];
                    sourceChars[targetIndex] = temp;
                } else if (sourceType === 'target' && targetType === 'target') {
                    if (targetChars[targetIndex] !== null) {
                        const temp2 = targetChars[sourceIndex];
                        targetChars[sourceIndex] = targetChars[targetIndex];
                        targetChars[targetIndex] = temp2;
                    } else {
                        targetChars[targetIndex] = targetChars[sourceIndex];
                        targetChars[sourceIndex] = null;
                    }
                } else if (sourceType === 'source' && targetType === 'target') {
                    if (targetChars[targetIndex] === null) {
                        targetChars[targetIndex] = sourceChars[sourceIndex];
                        sourceChars.splice(sourceIndex, 1);
                    } else {
                        const temp3 = sourceChars[sourceIndex];
                        sourceChars[sourceIndex] = targetChars[targetIndex];
                        targetChars[targetIndex] = temp3;
                    }
                } else if (sourceType === 'target' && targetType === 'source') {
                    if (sourceChars[targetIndex] === undefined) {
                        self.dragData = null;
                        self.render();
                        return;
                    }
                    const temp4 = targetChars[sourceIndex];
                    targetChars[sourceIndex] = sourceChars[targetIndex];
                    sourceChars[targetIndex] = temp4;
                }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('✅ SentenceBuilder loaded');
});
        ...
        <script>
            ...
            function speakWordWithEnglishAndCantonesePopup(word, meaning, onComplete) {
                ...
                function speakNext() {
                    ...
                    if (step === 0) {
                        ...
                        if (voice) utterance.voice = voice;
                        let completed = false;
                        utterance.onend = () => { if (completed) return;
                            completed = true;
                            repeatCount++; if (repeatCount < 3) { setTimeout(speakNext, 450); } else { step = 1;
                                repeatCount = 0;
                                setTimeout(speakNext, 450); } };
                        utterance.onerror = () => { if (completed) return;
                            completed = true;
                            step = 1;
                            repeatCount = 0;
                            setTimeout(speakNext, 450); };
                        try { speechSynthesis.speak(utterance); } catch (e) { step = 1;
                            repeatCount = 0;
                            setTimeout(speakNext, 450); }
                    } else if (step === 1) {
                        const utterance = new SpeechSynthesisUtterance(meaning);
            ...
            function toggleWordsAutoPlayPopup() {
                ...
                if (!wordsAutoPlayStatePopup.isPlaying && !wordsAutoPlayStatePopup.isPaused) {
                    ...
                    wordsAutoPlayStatePopup.isPlaying = false;
                    if (wordsAutoPlayStatePopup.timeoutId) { clearTimeout(wordsAutoPlayStatePopup.timeoutId);
                        wordsAutoPlayStatePopup.timeoutId = null; }
        ...
        function onDragEnd(e) {
            ...
            if (target && target.type !== null && target.index !== null) {
                ...
                if (sourceType === 'source') {
                    ...
                    if (sourceWord === undefined) { dragData = null;
                        renderBuilder(); return; }
                } else {
                    sourceWord = targetWords[sourceIndex];
                    if (sourceWord === null) { dragData = null;
                        renderBuilder(); return; }
                }

                if (targetType === 'source') {
                    targetWord = sourceWords[targetIndex];
                    if (targetWord === undefined) { dragData = null;
                        renderBuilder(); return; }
                } else {
