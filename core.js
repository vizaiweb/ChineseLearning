// ====================== 核心資料載入模組 ======================
// 此檔案包含所有與 Excel 讀取相關的核心函數。
// 所有 UI 相關的修改請在 UI.js 中進行。
...
async function loadFileListByLevel(level) {
    ...
    try {
        ...
        resetDayArea();
        if (typeof stopAllReading === 'function') stopAllReading();
...
async function parseExcelBufferAndLoad(buf, sourceLabel = "file") {
    try {
        ...
        if (filteredWords.length) {
            if (typeof showWord === 'function') showWord();
            if (typeof updateInfoTip === 'function') updateInfoTip();
        } else {
        ...
        if (allSentences.length === 0) {
            ...
            currentSentenceIdx = 0;
            if (typeof updateSentenceUI === 'function') updateSentenceUI();
            if (typeof updateSentenceStats === 'function') updateSentenceStats();
        }
...
function bindCoreEvents() {
    ...
    if (filterBtn) {
        ...
        filterBtn.addEventListener('click', function(e) {
            ...
            if (typeof filterByDay === 'function') filterByDay();
        });
    }
    ...
    if (savedLevel && savedFile && levelSelect) {
